...
function WebIOPi() {
    ...
    for(var i = 0; i < scripts.length; i++) {
       console.log("scripts[i].src=" + scripts[i].src);
       if(scripts[i].src!=""){
...
/*
	//动态引入jquery ，并执行页面的初始化
	var jquery = document.createElement('script');
	jquery.type = 'text/javascript';
	jquery.src = 'https://code.jquery.com/jquery-1.12.4.js';
	// if (!isMobile()) {
...
WebIOPi.prototype.init = function() {
	$.getJSON(w().context + "gpioInfo.ajax", function(data) {
		console.log("gpioInfo.ajax=" + data);
...
WebIOPi.prototype.refreshGPIO = function (repeat) {
	$.getJSON(w().context + "gpioInfoDetail ajax", function(data) {
...
WebIOPi.prototype.checkVersion = function () {
	...
	$.get(w().context + "version.ajax", function(data) {
...
WebIOPi.prototype.callMacro = function (macro, args, callback) {
	console.log("callMacro:" + " macro=" + macro + " args=" + args + " callback=" + callback);
	if (args == undefined) {
		args = "";
	}
